...
class MBModel:
    def __init__(
        ...
        *,
        ...
        prior_type: str = "spike_slab",  # choice of prior
        edge_prior_prob: float = 0.1,  # customizable prior
    ) -> None:
        ...
        # Expression data — robust dense conversion (handles SciPy sparse arrays/matrices)
        X = adata.X
        if sp.issparse(X):
            X = X.toarray()
        else:
    ...
    def export_networks(
        ...
        if self.prior_type == "spike_slab":
            # Extract edge probabilities (now continuous in [0, 1])
        ...
        if not return_cell_type_specific:
            ...
            result_data: dict[str, np.ndarray | list] = {
            ...
            # Add weights
            if "tau" in q_dict:  # spike-slab
                tau_mid = q_dict["tau"][len(quantiles) // 2]
                result_data["weight"] = tau_mid[keep_mask].cpu().numpy()
                if include_confidence:
                    result_data["weight_lower"] = q_dict["tau"][0][keep_mask].cpu().numpy()
                    result_data["weight_upper"] = q_dict["tau"][-1][keep_mask].cpu().numpy()
            else:  # horseshoe
                result_data["weight"] = edge_weights[keep_mask].cpu().numpy()
            ...
            phi_mid = q_dict["phi"][len(quantiles) // 2]  # Shape: (n_types, n_edges)

            for ct_idx, ct_name in enumerate(self.ctypes):
                # Cell-type specific weights
                if self.prior_type == "spike_slab":
                    tau_mid = q_dict["tau"][len(quantiles) // 2]
                    ct_weights = edge_probs * (tau_mid + phi_mid[ct_idx])
                    prob_vals = edge_probs
                else:
                    ct_weights = edge_weights + phi_mid[ct_idx]
                    prob_vals = None
                ...
                ct_data: dict[str, np.ndarray | list] = {
                    "gene_i": [self.genes[i] for i in i_indices],
                    "gene_j": [self.genes[j] for j in j_indices],
                    "weight": ct_weights[ct_keep].cpu().numpy(),
                }

                if self.prior_type == "spike_slab" and prob_vals is not None:
                    ct_data["probability"] = prob_vals[ct_keep].cpu().numpy()
